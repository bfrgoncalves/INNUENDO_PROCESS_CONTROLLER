...
def process_chewbbaca(key_value_args, parameters, user_folder, workflow, current_specie, workflow_name, sampleName, current_user_name, current_user_id):
	...
	status_definition += ' sleep 8; curl -L -X GET "http://'+config["FRONTEND_SERVER_IP"]+'/app/api/v1.0/jobs/?database_to_include='+ current_specie +'&job_id=SLURM_ARRAY_JOB_ID_STEPIDMINUS1&pipeline_id=' + workflow["pipeline_id"] + '&procedure_name='+ workflow_name +'&process_id=' + workflow["process_id"] + '&process_position=' + workflow["process_id"] + '&project_id=' + workflow["project_id"] + '&sample_name='+sampleName+'&current_user_name='+current_user_name+'&current_user_id='+current_user_id+'";'
