...
def process_innuca(key_value_args, parameters, user_folder, workflow, current_specie, workflow_name, sampleName, current_user_name, current_user_id, homedir):
	...
	#Add to db in case of INNUca warning
	...
	#Add to db in case of INNUca fail (by the program itself)
	status_definition += ' sleep 15; curl -L -X GET "http://'+config["FRONTEND_SERVER_IP"]+'/app/api/v1.0/jobs/?database_to_include='+ current_specie +'&job_id=SLURM_ARRAY_JOB_ID_STEPIDMINUS1&pipeline_id=' + workflow["pipeline_id"] + '&procedure_name='+ workflow_name +'&process_id=' + workflow["process_id"] + '&process_position=' + workflow["process_id"] + '&project_id=' + workflow["project_id"] + '&sample_name='+sampleName+'&current_user_name='+current_user_name+'&current_user_id='+current_user_id+'&from_process_controller=true&homedir='+homedir+'";'
	status_definition += ' else '
...
def process_pathotyping(key_value_args, parameters, user_folder, workflow, current_specie, workflow_name, sampleName, current_user_name, current_user_id, homedir):
	...
	after_application_steps += ' ln -s ' + os.path.join(str(user_folder),'SLURM_ARRAY_JOB_ID') + '/patho_typing.extended_report.txt ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/extended_report.txt;'
	...
	status_definition += ' python job_processing/get_program_input.py --project ' + workflow["project_id"] + ' --pipeline ' + workflow["pipeline_id"] + ' --process ' + workflow["process_id"] + ' -v1 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/run_info.json -v2 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/run_stats.json -v3 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/extended_report.txt -v4 ' +os.path.join(str(user_folder),'SLURM_ARRAY_JOB_ID', 'log_output_PathoTyping.txt')+ ' -v5 true -t output;'
	status_definition += ' sleep 15; curl -L -X GET "http://'+config["FRONTEND_SERVER_IP"]+'/app/api/v1.0/jobs/?database_to_include='+ current_specie +'&job_id=SLURM_ARRAY_JOB_ID_STEPIDMINUS1&pipeline_id=' + workflow["pipeline_id"] + '&procedure_name='+ workflow_name +'&process_id=' + workflow["process_id"] + '&process_position=' + workflow["process_id"] + '&project_id=' + workflow["project_id"] + '&sample_name='+sampleName+'&current_user_name='+current_user_name+'&current_user_id='+current_user_id+'&from_process_controller=true&homedir='+homedir+'";'
	status_definition += ' else '
	...
	status_definition += ' python job_processing/get_program_input.py --project ' + workflow["project_id"] + ' --pipeline ' + workflow["pipeline_id"] + ' --process ' + workflow["process_id"] + ' -v1 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/run_info.json -v2 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/run_stats.json -v3 ' + os.path.join(str(user_folder),"SLURM_ARRAY_JOB_ID") + '/PathoTyping_SLURM_ARRAY_JOB_ID_STEPID/extended_report.txt -v4 ' +os.path.join(str(user_folder),'SLURM_ARRAY_JOB_ID', 'log_output_PathoTyping.txt')+ ' -v5 false -t output;'
	status_definition += ' fi;'
...
def process_chewbbaca(key_value_args, parameters, user_folder, workflow, current_specie, workflow_name, sampleName, current_user_name, current_user_id, homedir):
	...
	prev_application_steps += "rm -rf ~/chewBBACA_schemas/"+schema_to_use+"; ";
	...
	#add to db in case chewBBACA as warnings
	status_definition += ' sleep 15; curl -L -X GET "http://'+config["FRONTEND_SERVER_IP"]+'/app/api/v1.0/jobs/?database_to_include='+ current_specie +'&job_id=SLURM_ARRAY_JOB_ID_STEPIDMINUS1&pipeline_id=' + workflow["pipeline_id"] + '&procedure_name='+ workflow_name +'&process_id=' + workflow["process_id"] + '&process_position=' + workflow["process_id"] + '&project_id=' + workflow["project_id"] + '&sample_name='+sampleName+'&current_user_name='+current_user_name+'&current_user_id='+current_user_id+'&from_process_controller=true&homedir='+homedir+'";'
	status_definition += ' else '
